...
def open_dataset_ccs():
    ...
    dataset = pd.read_excel(io = url,  header=0)
    # print(dataset.head())
    return dataset
...
############   UMAP test   #####################
    ...
    # number of threads
    threads_list = [1, 2, 4]
    path = r'C:/Users/15754/Desktop/LDATE2990 - Master Thesis/Project/results/datasets/Iris/t-sne/'
        ...
        for min_grad_norm_value in tot_tsne_min_grad_norm_group:
            for n_iter_value in tot_tsne_n_iter_group:
                for early_exaggeration_value in tot_tsne_early_exaggeration_group:
                    ...
                    print("Applying " + str(algorithm)+ " for value in variable with perp = {perp}, min_grad_norm = {min_grad_norm}, n_iter = {n_iter}, early_exaggeration = {early_exaggeration}".format(perp = perp_value, min_grad_norm = min_grad_norm_value, n_iter = n_iter_value, early_exaggeration = early_exaggeration_value))
                    ...
                    List_time.append([perp_value, min_grad_norm_value, n_iter_value, early_exaggeration_value, elapsed_time])
                    List_auc.append([perp_value, min_grad_norm_value, n_iter_value, early_exaggeration_value, auc_value])
                    ...
                    if auc_value > best_auc_score:#找到表现最好的参数
                        ...
                        best_parameters = {str(variable1) :perp_value, str(variable2):min_grad_norm_value, str(variable3): n_iter_value, str(variable4):early_exaggeration_value}
    ...
    # print("auc before", best_auc_score)
    # print("type of auc list", type(best_auc_score))
    # print("para before", best_parameters)
    # print("type of para list", type(best_parameters))

    # test1 = {str(variable_target1): best_auc_score}
    # print(test1)
    # print("type of auc list after transform", type(test1))

    # test2 = best_parameters.update({str(variable_target1): best_auc_score})
    # test3 = test1 + test2
    # print(test2)

    # test3 = np.array(test1 + test2)


    # print(test3)
    #
    # np.save(path +'geekfile',test3)

    # np.savetxt(path +'_'+ str(dataset_name) + str(algorithm) + '_best_result_parameters.csv', test3, delimiter=',')
    ...
    # test3.to_csv (name_auc, index = False, header=True)
