...
% The negative sampling algorithm derives edge sampling algorithm in large-scale information network embedding.
...
\begin{figure}[H]
\centering  %图片全局居中
\subfigure[edge sampling]
{
\label{Fig.sub.1}
\includegraphics[width=7cm,height=5cm\textwidth]{images/image_largevis_line_1.PNG}}
\subfigure[negative sampling]
{
\label{Fig.sub.2}
\includegraphics[width=7cm,height=5cm\textwidth]{images/image_largevis_ld_visulization_1.PNG}}
...
\noindent Where $v^K_i$ and $n^K_i$ index the K nearest neighbors of high dimension data point $x_i$ and $y_i$ in the HD and LDS, with $Q_{NX} (K) = \sum_{i \in I} \mid v^K_i \cap n^K_i \mid / (KN)$ This criterion varies between 0 and 1 and measures the average normalized agreement between corresponding K nearest neighborhoods in the HD and LD spaces\cite{ref18}. Its expectation for random LD points becomes $K/ (N − 1)$. With $R_{NX} (K) = ((N − 1)Q_{NX} (K) − K) /(N − 1 − K)$ for comparing different neighbour sizes \cite{ref11}, the curve of the DR quality could be displayed with a log-scale for K as closer neighbors. \\ 

for $R_{NX}$. This criterion varies between 0 and 1 and measures the average normalized agreement between corresponding K nearest neighborhoods in the HD and LD spaces\\