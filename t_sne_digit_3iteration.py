...
if __name__ == '__main__':
    ...
    Ltheta = [0.7, 0.8, 0.9]
    path = 'C:/Users/15754/Desktop/LDATE2990 - Master Thesis/Project/results/t-sne/digit/graph_iteration/'
    #C:\Users\15754\Desktop\LDATE2990 - Master Thesis\Project\results\t-sne\digit\graph_iteration
    ...
    # iterations = [1]
    #neg_group = [3,5,7]
    #tot_perplexity_gourp = [[10,15,20],[25,30,35],[40,45,50]]
    tot_perplexity_gourp = [[55, 60, 65],[70, 75, 88]]
    Lmedw = [1.5, 1.0, 1.0]
    Lsdots = [14, 12, 12]
    #perplexity_gourp = [10,30,50]
    learning_rate_group = [100,200,300]

    print("Loading the test data set.")
    ######   test    #######
    #train_df = read_csv("C:/Users/15754/Desktop/heart.csv", index_col=0)
    ...
    #id_subs = rand_state.choice(a=X_hds.shape[0], size=150, replace=False)
    ...
    for iteration in iterations:
        round_mark = int(0)
        for perplexity_group in tot_perplexity_gourp:
            round_mark += 1
            print("iteration_",iteration)
            print("round_",round_mark)
            # For each theta
            # For each perplexity
            for perplexity in perplexity_group:
                #perplexity = 30
                #theta = 0.7
                  ####
        ...
        ### 2）读一遍paper，重点是算法原理与模型评估这两块
                ...
                print("Computing the DR quality of the result of t-SNE with threshold perplexity = {perplexity}".format(perplexity=perplexity))
                ...
                L_rnx.append(rnxk)
                #L_kg.append(kg)
                Ltime.append("{running_time} ($perplexity= {perplexity}$)".format(running_time = elapsed_time, perplexity = perplexity))
                Lleg_rnx.append("{a} t-SNE ($perplexity= {perplexity}$)".format(a=int(round(auc_rnx*1000))/1000.0, perplexity= perplexity))
                #Lleg_kg.append("{a} t-SNE ($perplexity= {perplexity}$)".format(a=int(round(auc_kg*1000))/1000.0, perplexity= perplexity))
                Lls.append('solid')
            list_L_rnx = L_rnx[-3:]
            list_Lleg_rnx = Lleg_rnx[-3:]
            try:
                name = 'perplexity_iteration_' + str(iteration) + '_round_' + str(round_mark) + '_time_' + str(start_time)
                #print("name", name)
                viz_qa(path=path, name=name, Ly=list_L_rnx, Lmarkers=Lmarkers, Lcols=Lcols, Lleg=list_Lleg_rnx, Lls=Lls, Lmedw=Lmedw, Lsdots=Lsdots, tit='DR quality', xlabel='perplexity_size $K$', ylabel='$R_{NX}(K)$')

            except:
                name = 'perplexity_iteration_' + str(iteration) + '_time_' + str(start_time)
                #print("name", name)
                viz_qa(path=path, name=name, Ly=list_L_rnx, Lmarkers=Lmarkers, Lcols=Lcols, Lleg=list_Lleg_rnx, Lls=Lls, Lmedw=Lmedw, Lsdots=Lsdots, tit='DR quality', xlabel='perplexity_size $K$', ylabel='$R_{NX}(K)$')
